...
export default function Pendant() {
	...
	const { data, error } = useSWR(
		() => query.id && `/api/pendants/${query.id}`,
		fetcher
	);

	if (error) return <div>{error.message}</div>;
	if (!data) return <div>Retrieving pendants data...</div>;
...
export async function getStaticProps({ params }) {
	const res = await fetch(
		process.env.NEXT_PUBLIC_URL + `/api/pendants/${params.id}`
	);
	const data = await res.json();
	const pendants = data;
	return {
...
export async function getStaticPaths() {
	const res = await fetch(process.env.NEXT_PUBLIC_URL + "/api/pendants");
	const data = await res.json();
