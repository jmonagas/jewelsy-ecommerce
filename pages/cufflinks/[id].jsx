...
export default function Cufflink() {
	const { query } = useRouter();
	const { data, error } = useSWR(
		() => query.id && `/api/cufflinks/${query.id}`,
		fetcher
	);

	if (error) return <div>{error.message}</div>;
	if (!data) return <div>Retrieving cufflinks data...</div>;
...
export async function getStaticProps({ params }) {
	const res = await fetch(
		process.env.NEXT_PUBLIC_URL + `/api/cufflinks/${params.id}`
	);
	const data = await res.json();
	const cufflinks = data;
	return {
		props: { cufflinks },
		revalidate: 86400,
	};
...
export async function getStaticPaths() {
	const res = await fetch(process.env.NEXT_PUBLIC_URL + "/api/cufflinks");
	const data = await res.json();
