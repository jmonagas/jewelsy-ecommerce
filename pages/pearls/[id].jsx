...
export default function Pearl() {
	...
	const { data, error } = useSWR(
		() => query.id && `/api/pearls/${query.id}`,
		fetcher
	);

	if (error) return <div>{error.message}</div>;
	if (!data) return <div>Retrieving pearls data...</div>;
...
export async function getStaticProps({ params }) {
	const res = await fetch(
		process.env.NEXT_PUBLIC_URL + `/api/pearls/${params.id}`
	);
	const data = await res.json();
	const pearls = data;
	return {
...
export async function getStaticPaths() {
	const res = await fetch(process.env.NEXT_PUBLIC_URL + "/api/pearls");
	const data = await res.json();
